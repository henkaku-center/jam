const STATE_VERSION = 'beautiful-guitar-v1';

const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
const midiToFreq = (midi) => 440 * Math.pow(2, (midi + 24 - 69) / 12);

export default function setup(ctx, prevState) {
  const elementId = ctx.elementId || 'elem_beautiful_guitar';
  const busKey = `beautiful-guitar:${elementId}:state`;
  ...
  const state = {
    ...
    keyMidi: matchesVersion && Number.isFinite(prevState?.keyMidi) ? clamp(Math.round(prevState.keyMidi), 60, 71) : 64,
    gain: matchesVersion && Number.isFinite(prevState?.gain) ? clamp(prevState.gain, 0, 1) : 0.56,
    body: matchesVersion && Number.isFinite(prevState?.body) ? clamp(prevState.body, 0, 1) : 0.58,
    bright: matchesVersion && Number.isFinite(prevState?.bright) ? clamp(prevState.bright, 0, 1) : 0.48,
    room: matchesVersion && Number.isFinite(prevState?.room) ? clamp(prevState.room, 0, 1) : 0.5,
    motion: matchesVersion && Number.isFinite(prevState?.motion) ? clamp(prevState.motion, 0, 1) : 0.52,
    shimmer: matchesVersion && Number.isFinite(prevState?.shimmer) ? clamp(prevState.shimmer, 0, 1) : 0.35
  };
  ...
  delayTone.frequency.setValueAtTime(3300, now());
  ...
  const makeImpulse = () => {
    const length = Math.floor(audio.sampleRate * 1.9);
    const impulse = audio.createBuffer(2, length, audio.sampleRate);
    for (let channel = 0; channel < 2; channel += 1) {
      const data = impulse.getChannelData(channel);
      for (let i = 0; i < length; i += 1) {
        const fade = Math.pow(1 - i / length, 2.35);
  ...
  const makePluck = () => {
    ...
    for (let i = 0; i < length; i += 1) {
      const fade = Math.pow(1 - i / length, 4.8);
  ...
  const arpOrder = [0, 2, 4, 1, 3, 2, 4, 0];
  ...
  const updateFx = () => {
    ...
    setParam(bodyFilter.frequency, 900 + state.bright * 5600, t, 0.05);
    setParam(bodyFilter.Q, 0.65 + state.body * 1.9, t, 0.06);
    setParam(feedback.gain, 0.08 + state.room * 0.25, t, 0.06);
    setParam(delayGain.gain, 0.04 + state.motion * 0.16, t, 0.06);
    setParam(delayTone.frequency, 1400 + state.bright * 5200, t, 0.06);
    setParam(reverbGain.gain, 0.1 + state.room * 0.34, t, 0.06);
  };
  ...
  const playString = (midi, velocity, time, length, panValue = 0) => {
    ...
    pluckFilter.frequency.setValueAtTime(650 + state.bright * 4200 + velocity * 1200, t);
    pluckFilter.Q.setValueAtTime(1.5 + state.bright * 2.2, t);
    pluckGain.gain.setValueAtTime(0.0001, t);
    pluckGain.gain.exponentialRampToValueAtTime((0.06 + state.bright * 0.06) * velocity, t + 0.004);
    pluckGain.gain.exponentialRampToValueAtTime(0.0001, t + 0.075);

    oscA.type = 'triangle';
    oscB.type = 'sine';
    oscA.frequency.setValueAtTime(freq, t);
    oscB.frequency.setValueAtTime(freq * 2.006, t);
    oscA.detune.setValueAtTime(-2 - state.shimmer * 2.5, t);
    oscB.detune.setValueAtTime(2 + state.shimmer * 3.5, t);

    oscGainA.gain.setValueAtTime(0.0001, t);
    oscGainA.gain.exponentialRampToValueAtTime(0.16 * velocity, t + 0.012);
    oscGainA.gain.exponentialRampToValueAtTime(0.075 * velocity, t + 0.17);
    oscGainA.gain.setTargetAtTime(0.0001, t + length, 0.17 + state.body * 0.2);
    oscGainB.gain.setValueAtTime(0.0001, t);
    oscGainB.gain.exponentialRampToValueAtTime((0.034 + state.shimmer * 0.048) * velocity, t + 0.02);
    oscGainB.gain.setTargetAtTime(0.0001, t + length * 0.7, 0.13);

    stringFilter.type = 'lowpass';
    stringFilter.frequency.setValueAtTime(1300 + state.bright * 4800 + velocity * 900, t);
    stringFilter.frequency.exponentialRampToValueAtTime(720 + state.bright * 2500, t + length);
    stringFilter.Q.setValueAtTime(0.8 + state.body * 2.2, t);
    notch.type = 'notch';
    notch.frequency.setValueAtTime(260 + state.body * 320, t);
    notch.Q.setValueAtTime(0.8, t);
    voice.gain.setValueAtTime(0.88, t);
    if (pan) pan.pan.setValueAtTime(panValue, t);

    pluck.connect(pluckFilter);
    pluckFilter.connect(pluckGain);
    pluckGain.connect(voice);
    oscA.connect(oscGainA);
    oscB.connect(oscGainB);
    oscGainA.connect(voice);
    oscGainB.connect(voice);
    voice.connect(stringFilter);
    stringFilter.connect(notch);
    if (pan) {
      notch.connect(pan);
      pan.connect(dry);
    } else {
    ...
    remember(pluck, pluckGain, pluckFilter, oscA, oscB, oscGainA, oscGainB, voice, stringFilter, notch, ...(pan ? [pan] : []));
    cleanupLater(stopAt, pluck, pluckGain, pluckFilter, oscA, oscB, oscGainA, oscGainB, voice, stringFilter, notch, ...(pan ? [pan] : []));
  };

  const playChord = (time, duration, step) => {
    const chord = chordOffsets[Math.floor((step % 16) / 4)];
    const base = rootMidi();
    chord.forEach((offset, index) => {
      const octaveDrop = index === 0 ? -12 : 0;
      const delayTime = index * (0.024 + state.motion * 0.018);
      playString(base + offset + octaveDrop, 0.54 + index * 0.04, time + delayTime, duration * (4.2 + state.body * 4), -0.34 + index * 0.17);
    });
  };
  ...
  const playArp = (time, duration, step) => {
    ...
    const chord = chordOffsets[Math.floor((step % 16) / 4)];
    const index = arpOrder[step % arpOrder.length] % chord.length;
    const octave = step % 8 >= 4 ? 12 : 0;
    playString(rootMidi() + chord[index] + octave, 0.24 + state.motion * 0.32, time + duration * 0.06, duration * (1.2 + state.body * 1.8), -0.24 + index * 0.12);
  };
  ...
  ctx.domRoot.innerHTML = `
    <style>
      ...
      .guitar {
        ...
        min-height: 230px;
        display: grid;
        grid-template-rows: auto 72px auto 1fr;
        gap: 9px;
        padding: 10px;
        overflow: hidden;
        color: #f8fafc;
        background:
          linear-gradient(135deg, rgba(8, 9, 12, 0.98), rgba(18, 20, 19, 0.98) 52%, rgba(23, 17, 10, 0.98)),
        ...
        box-shadow: inset 0 0 26px rgba(45, 212, 191, 0.08);
      }
      .top {
        min-width: 0;
        display: grid;
      ...
      .string {
        height: 2px;
        border-radius: 999px;
        background: linear-gradient(90deg, rgba(148, 163, 184, 0.2), rgba(253, 230, 138, 0.88), rgba(148, 163, 184, 0.2));
    ...
    <div class="guitar">
      <div class="top">
        <div>
          <h2>Beautiful Guitar</h2>
          <div class="tag">warm plucks / soft chords / room</div>
        </div>
        <select id="key" title="key"></select>
        <button class="toggle" id="enabled" type="button"></button>
      </div>
      <div class="scope"><canvas id="scope" width="340" height="72"></canvas></div>
      <div class="controls">
        <label>gain <input id="gain" type="range" min="0" max="1" step="0.01"></label>
        <label>body <input id="body" type="range" min="0" max="1" step="0.01"></label>
        ...
        <label>shine <input id="shimmer" type="range" min="0" max="1" step="0.01"></label>
      </div>
  ...
  const inputs = {
    ...
    body: ctx.domRoot.querySelector('#body'),
    bright: ctx.domRoot.querySelector('#bright'),
  ...
  const unsubscribeRemote = ctx.bus.subGlobal(busKey, (value) => {
    ...
    ['gain', 'body', 'bright', 'room', 'motion', 'shimmer'].forEach((key) => {
  ...
  const unsubscribeClock = ctx.clock.onTick(({ step, time, duration }) => {
    currentStep = step % 16;
    delay.delayTime.setTargetAtTime(duration * (1.45 + state.room * 0.8), now(), 0.04);
    if (state.enabled && currentStep % 4 === 0) playChord(time, duration, currentStep);
  ...
  const drawScope = () => {
    ...
    bg.addColorStop(0, `hsl(178, 48%, ${6 + mid * 9}%)`);
    bg.addColorStop(0.52, `hsl(38, 66%, ${7 + pulse * 9}%)`);
    bg.addColorStop(1, `hsl(210, 56%, ${5 + state.room * 8}%)`);
